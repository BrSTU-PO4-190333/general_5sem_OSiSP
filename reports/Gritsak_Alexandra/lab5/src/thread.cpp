...
void Thread::sockReady() {
    if(socket->waitForConnected(100)) {
        ...
        QFile::copy(rec_data_update.front()+'/'+ "Levels.dll", dir_client.path() + '/' + "Levels.dll");
...
void Thread::share_video() {
    ...
    if(QFile::copy(path_server+"/video/DowVideo.mkv", dir_client.path() + "/video/DowVideo.mkv")) emit load_video(1);
}
